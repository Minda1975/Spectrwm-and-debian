...
term="st -n floatterm -g 150x50 -c"
#term="kitty --class"
scratch () {
    case $1 in
        pad)
        ...
        pulse)
            ...
            if [ -z "$winclass" ]; then
                $term scratchpulse -e pulsemixer 2> /dev/null
            else
        ...
        vifm)
            ...
            if [ -z "$winclass" ]; then
                $term scratchvifm -e vifmrun 2> /dev/null
            else
        ...
        sfm)
            ...
            if [ -z "$winclass" ]; then
                $term scratchsfm -e sfm 2> /dev/null
            else
        ...
        fzf)
            ...
            if [ -z "$winclass" ]; then
                $term scratchfzf -e fzf-launcher.sh 2> /dev/null
            else
